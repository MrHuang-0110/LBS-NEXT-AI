...
/* FLASH起始地址 */
#define STM32_FLASH_SIZE        0x9000         /* STM32 FLASH 总大小 */
#define STM32_FLASH_BASE        0x08037000      /* STM32 FLASH 起始地址 */
...
uint8_t stmflash_write(uint32_t waddr, uint16_t *pbuf, uint16_t length);   /* 在FLASH 指定位置, 写入指定长度的数据(自动擦除) */
uint8_t flash_erase_sector(uint32_t saddr);
/* 测试函数 */
void test_write(uint32_t waddr, uint16_t wdata);                        /* 测试写数据(写1个字) */
